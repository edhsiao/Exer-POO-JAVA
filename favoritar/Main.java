...
public class Main {
    ...
    public static void addFavorite(String pageName, String userLogin) {
        ...
        db.getCollection("users")
                .findOneAndUpdate(Filters.eq("login", userLogin),
                        new Document("$addToSet", new Document("favorites", pageName)));
    }

    public static void removeFavorite(String pageName, String userLogin) {
        // 1. Seleciona a coleção de usuários
        // 2. Encontra o primeiro usuário com login igual ao fornecido e o atualiza de acordo com
        ...
        db.getCollection("users")
                .findOneAndUpdate(Filters.eq("login", userLogin),
                        new Document("$pull", new Document("favorites", pageName)));
    }
    ...
    public static List<Document> favorites(String userLogin, int size) {
        ...
        // de ObjectIds
        List<String> pageNames = db.getCollection("users")
        ...
        // 4. Limita o número de resultados retornados
        // 5. Salva todos os resultados dentro de um ArrayList
        return db.getCollection("posts")
                .find(Filters.in("page", pageNames))
    ...
    public static void main(String[] args) throws Throwable {
        ...
        System.out.println("\n" + getUser("teste@email.com"));

        // Seleciona os posts, ordena-os de forma aleatória, pula os 3 primeiros e
        // executa uma função anônima para cada um que adiciona suas páginas nos favoritos
        ...
        randomPosts.forEach((e) -> addFavorite(e.getString("page"), "teste@email.com"));

        // Recupera os favoritos do usuário de login fornecido e imprime na tela similar a anteriormente
        favorites("teste@email.com", 10).forEach(e -> System.out.println(e.toJson()));

        // Imprime o JSON do usuário manipulado para verificação
        System.out.println("\n" + getUser("teste@email.com"));

        // Remove as duas ultimas paginas dos favoritos do usuário anteriormente manipulado
        randomPosts.stream().skip(1).forEach(e -> removeFavorite(e.getString("page"), "teste@email.com"));
        ...
        randomPosts.forEach(e -> removeFavorite(e.getString("page"), "teste@email.com"));
